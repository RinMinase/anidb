...
export class LastwatchComponent implements OnInit {
	...
	private formatData(
		dataDateFinished: Array<any>,
		dataRewatchLast: Array<any>,
	) {
		...
		this.stats.daysSinceLastDateCounted = format(
			new Date().getTime() - dateLast.getTime(),
			"DDD",
		);
		this.stats.daysSinceLastAnime = format(
			new Date().getTime() - dateFirst.getTime(),
			"DDD",
		);
		this.stats.titlesPerWeek = ((sortedData.length / dateDiffLast) * 7).toFixed(
			2,
		);
		this.stats.singleSeasonPerWeek = (
			(singleSeason / dateDiffLast) *
			7
		).toFixed(2);
		this.stats.episodesPerDay = (
			this.stats.totalEpisodes / dateDiffLast
		).toFixed(2);
		...
		sortedData.forEach((value: any) => {
			if (value.dateFinished === "") {
				...
				value.dateFinished = format(
					fromUnixTime(value.dateFinished),
					this.dateFormat,
				);
			}

			if (value.rewatchLast) {
				value.rewatchLast = format(
					fromUnixTime(value.rewatchLast),
					this.dateFormat,
				);
			}
